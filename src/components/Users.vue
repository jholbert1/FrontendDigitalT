<template>
  <v-container>
    <div class="d-flex justify-end">
      <v-icon @click="$router.go(-1)">mdi-arrow-left</v-icon>
    </div>
    <v-row>
      <v-col cols="12" md="4">
        <v-btn
          @click="
            () => {
              create = true;
              dataUser = {};
              action = 0;
            }
          "
          large
          rounded
          color="primary"
        >
          Crear Usuario
        </v-btn>
        <v-btn
          v-if="arrayDelete.length > 0"
          @click="deleteUsers()"
          large
          rounded
          color="primary"
        >
          Eliminar Usuarios
        </v-btn>
      </v-col>
      <v-col cols="12" md="12">
        <v-data-table
          v-if="!showDetailsOrder"
          :headers="headers"
          :items="dataResponse"
          :items-per-page="5"
          class="elevation-0"
          hide-default-footer
          disable-pagination
          width="100%"
          tile
        >
          <template v-slot:[`item.action`]="{ item }">
            <v-tooltip bottom>
              <template v-slot:activator="{ on, attrs }">
                <v-btn
                  @click="
                    () => {
                      create = true;
                      dataUser = { ...item };
                      action = 1;
                    }
                  "
                  icon
                  color="primary"
                  dark
                  v-bind="attrs"
                  v-on="on"
                >
                  <v-icon>mdi-pencil</v-icon>
                </v-btn>
              </template>
              <span>editar</span>
            </v-tooltip>
            <v-btn
              v-if="authUser.id != item.id"
              @click="deleteItem(item)"
              icon
              :color="arrayDelete.includes(item.id) ? 'red' : 'primary'"
              dark
            >
              <v-icon>mdi-delete-forever</v-icon>
            </v-btn>
          </template>
        </v-data-table>
      </v-col>
    </v-row>
    <v-row justify="center">
      <v-col cols="6" md="4">
        <v-pagination
          v-model="page"
          :length="paginate.lastPage"
          circle
          color="#00A0E9"
        ></v-pagination>
      </v-col>
    </v-row>
    <register-update-component
      v-if="create"
      :active="create"
      :data="dataUser"
      :action="action"
      @dialog:change="handlerResponse"
    />
  </v-container>
</template>

<script>
import RegisterUpdate from "./Users/RegisterUpdate.vue";
export default {
  components: {
    "register-update-component": RegisterUpdate,
  },

  data() {
    return {
      loading: false,
      page: 1,
      per_page: 5,
      showDetailsOrder: false,
      headers: [
        {
          text: "Correo",
          align: "start",
          sortable: false,
          value: "email",
          class: "header-text",
        },
        { text: "Nombre", value: "names", class: "header-text" },
        { text: "Apellido", value: "last_names", class: "header-text" },
        { text: "Telefono", value: "phone", class: "header-text" },
        { text: "Direccion", value: "address", class: "header-text" },
        {
          text: "Fecha Nacimiento",
          value: "birth_date",
          class: "header-text",
        },
        { text: "Rol", value: "roles[0].description", class: "header-text" },
        { text: "Acción", value: "action", class: "header-text" },
      ],
      dataResponse: [],
      paginate: {},
      create: false,
      dataUser: {},
      arrayDelete: [],
      action: 0
    };
  },

  created() {
    this.getUsers();
  },

  watch: {
    page(val) {
      this.getUsers(val);
    },
  },

  computed: {
    authUser() {
      return this.$store.getters["auth/GET_USER"];
    },
  },

  methods: {
    async getUsers() {
      try {
        const request = {
          page: this.page,
          per_page: this.per_page,
        };
        const response = await this.$store.dispatch("users/GET_USERS", request);
        this.dataResponse = response.data.data.data;
        this.paginate = {
          lastPage: response.data.data.lastPage,
          page: response.data.data.page,
          perPage: response.data.data.perPage,
          total: response.data.data.total,
        };
        console.log(response);
      } catch (error) {
        console.log(error);
      }
    },

    deleteItem(item) {
      if (this.arrayDelete.length == 0) {
        this.arrayDelete.push(item.id);
      } else if (this.arrayDelete.includes(item.id)) {
        const position = this.arrayDelete.indexOf(item.id);
        this.arrayDelete.splice(position, 1);
      } else {
        this.arrayDelete.push(item.id);
      }
    },

    async deleteUsers() {
      try {
        await this.$store.dispatch("users/DELETE", {
          deleteArray: this.arrayDelete,
        });
        this.$snotify.success("eliminado exitosamente", "Exitos!");
        this.arrayDelete = [];
        this.getUsers();
      } catch (error) {
        console.log(error);
      }
    },

    handlerResponse(val) {
      if (val) {
        this.getUsers();
      }
      this.create = false;
    },
  },
};
</script>

<style>
</style>